...
class Insert(QMainWindow):
    ...
    def getInfo(self):
        ...
        if self.object=='zhiyeweihai':
            name = self.lineEdit_name.text()
            id = self.lineEdit_id.text()
            shangwangriqi = self.lineEdit_telephone.text()
            weixianjibie = self.comboBox_weixian.currentText()
            guanxiabumen = self.lineEdit_bumen.text()
        
            kongzhiyaoqiu = self.lineEdit_kongzhiyaoqiu.text()
            zerenren = self.lineEdit_zerenren.text()
            jianchariqi = self.lineEdit_jianchariqi.text()
   
        
            keys = ['id,','zhiyeshangwangmingchen,','weixianjibie,','guanxiabumen,','shangwangriqi,','kongzhiyaoqiu,','zerenren,','jianchariqi,']
            values_temp = [id,name,weixianjibie,guanxiabumen,shangwangriqi,kongzhiyaoqiu,zerenren,jianchariqi]
            values = ['\''+value+"\'," for value in values_temp]
