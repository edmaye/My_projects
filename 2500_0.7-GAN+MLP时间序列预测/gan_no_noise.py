...
def main():
    ...
    for epoch in range(epochs):
        ...
        for d in range(1):
            ...
            y_gen = G(x_train)      #  （x）输入生成器，得到预测结果
    ...
    plt.title('GAN_no_noise')
    plt.legend()
    plt.savefig(fname="result/GAN_no_noise.png")
    np.save('result/GAN_no_noise.npy',loss_train)   # 保存为npy文件,供不同方法对比
    print('GAN_no_noise——测试集均方误差：',loss_val[-1])
    plt.show()
